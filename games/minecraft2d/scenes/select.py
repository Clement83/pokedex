...
_GAME_ROOT = os.path.abspath(os.path.join(_HERE, '..'))            # minecraft2d/
_PROJ_ROOT = os.path.abspath(os.path.join(_GAME_ROOT, '..', '..'))  # pokedex/
for _p in [_GAME_ROOT, _PROJ_ROOT]:
...
def _build_world_bg():
    """Génère un monde décoratif pré-rendu dans un Surface (COLS×TS) × SCREEN_HEIGHT."""
    world = _world_mod.World(_BG_SEED)
    ROW_N = SCREEN_HEIGHT // _TS
    W     = COLS * _TS
...
def _draw_slot(screen, font_big, font_sm, rect, world, selected, confirm_del):
    ...
    if world is None:
        ...
        if confirm_del:
            ...
            seed_lbl = font_big.render("Seed : " + str(world["seed"]), True,
                                       _MC_YELLOW if selected else TEXT_COLOR)
            screen.blit(seed_lbl, (x + 10, y + 10))
            date_lbl = font_sm.render("Derniere partie : " + world["last_played"],
                                      True, _MC_GRAY)
...
def run(screen, joysticks):
    ...
    while True:
        ...
        cur_mine = (_joy_btn(joy, J1_BTN_MINE) or _joy_btn(joy, BTN_A) or
                    _joy_btn(joy, BTN_B)        or bool(keys[KB_J1_MINE]))
        ...
        for i in range(MAX_SLOTS):
            ...
            _draw_slot(screen, font_big, font_sm, (sx, sy, SLOT_W, SLOT_H),
                       worlds[i], selected == i, confirm_del == i)

        hints = ["MINE = Jouer / Creer", "MOD + MINE = Ecraser", "SELECT+START = Quitter"]
