...
class ResultHandler:
    ...
    def handle_success(self, target_pokemon_data, is_shiny):
        ...
        self._check_and_unlock_new_region(pokedex_id)
    ...
    def handle_fled(self, target_pokemon_data):
        ...
        self._check_and_unlock_new_region(target_pokemon_data[0])
        return "detail"

    def _check_and_unlock_new_region(self, pokedex_id):
        ...
        for gen, data in sorted(GENERATION_THRESHOLDS.items(), key=lambda item: item[1]['unlock_count']):
            if seen_count >= data['unlock_count'] and previous_max_pokedex_id < data['max_id']:
